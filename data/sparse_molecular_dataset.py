...
class SparseMolecularDataset:
    def load(self, filename, subset=1):
        ...
        self.train_idx = np.random.choice(
            self.train_idx, int(len(self.train_idx) * subset), replace=False
        )
        ...
        self.test_idx = np.random.choice(
            self.test_idx, int(len(self.test_idx) * subset), replace=False
        )
    ...
    def _genA(self, mol, connected=True, max_length=None):
        ...
        # Elements of A are bond types not only 0 and 1
        A[begin, end] = bond_type
        A[end, begin] = bond_type

        # Degree matrix is not calculated correctly as it is not a sum of 1s in the adjacency matrix
        # but a sum of bond types
        degree = np.sum(A[: mol.GetNumAtoms(), : mol.GetNumAtoms()], axis=-1)

        # This can only detect vertices with degree 0
        ...
        features = np.array(
            [
                [
                    ...
                    *[
                        a.GetValence(Chem.rdchem.ValenceType.EXPLICIT) == i
                        for i in range(9)
                    ],
                    *[int(a.GetHybridization()) == i for i in range(1, 7)],
                    *[
                        a.GetValence(Chem.rdchem.ValenceType.IMPLICIT) == i
                        for i in range(9)
                    ],
                    a.GetIsAromatic(),
                    a.GetNoImplicit(),
