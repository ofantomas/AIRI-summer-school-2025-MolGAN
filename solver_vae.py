...
class Solver(object):
    ...
    def build_model(self):
        ...
        self.V = Discriminator(
            self.d_conv_dim, self.m_dim, self.b_dim - 1, self.dropout_rate
        ).to(self.device)

        self.vae_optimizer = torch.optim.RMSprop(
            list(self.encoder.parameters()) + list(self.decoder.parameters()), self.g_lr
        )
    ...
    def restore_model(self, resume_iters):
        ...
        enc_path = os.path.join(
            self.model_dir_path, "{}-encoder.ckpt".format(resume_iters)
        )
        dec_path = os.path.join(
            self.model_dir_path, "{}-decoder.ckpt".format(resume_iters)
        )
    ...
    def get_reconstruction_loss(self, n_hat, n, e_hat, e):
        ...
        n_loss = torch.nn.CrossEntropyLoss(reduction="mean")(
            n_hat.view(-1, self.m_dim), n.view(-1)
        )
        e_loss = torch.nn.CrossEntropyLoss(reduction="mean")(
            e_hat.reshape((-1, self.b_dim)), e.view(-1)
        )
    ...
    def save_checkpoints(self, epoch_i):
        enc_path = os.path.join(
            self.model_dir_path, "{}-encoder.ckpt".format(epoch_i + 1)
        )
        dec_path = os.path.join(
            self.model_dir_path, "{}-decoder.ckpt".format(epoch_i + 1)
        )
    ...
    def train_or_valid(self, epoch_i, train_val_test="val"):
        ...
        for a_step in range(the_step):
            ...
            if train_val_test == "train" or train_val_test == "val":
                ...
                vae_loss_train = (
                    self.lambda_wgan * loss_vae + (1 - self.lambda_wgan) * loss_rl
                )
            ...
            if train_val_test == "sample":
                ...
                mol_f_name = os.path.join(
                    self.img_dir_path, "sample-mol-{}.png".format(epoch_i)
                )
            ...
            if train_val_test == "val":
                ...
                # Saving molecule images.
                mol_f_name = os.path.join(
                    self.img_dir_path, "mol-{}.png".format(epoch_i)
                )
                save_mol_img(mols, mol_f_name, is_test=self.mode == "test")
